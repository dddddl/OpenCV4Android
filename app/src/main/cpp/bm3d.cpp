//
// Created by liuliang on 2019/12/9.
//
    ...
    for (int i = 0; i < bn_r; i++) {
        for (int j = 0; j < bn_c; j++) {
            ...
            getSimilarPatch(block_noisy, data, sim_num,
                            i, j, bn_r, bn_c, int((nHard - kHard) / pHard) + 1, NHard,
                            tao_hard);//block matching
            ...
            aggregation(numerator_hd, denominator_hd, sim_idx_row, sim_idx_col, data, weight_hd,
                        kHard, kaiser);//aggregation using weigths
        }
    }
    ...
    for (int i = 0; i < bn_r; i++) {
        for (int j = 0; j < bn_c; j++) {
            ...
            for (int k = 0; k < sim_num.size(); k++)//calculate idx in the left-top corner
            {
                sim_idx_row.push_back(row_idx[sim_num[k] / bn_c]);
                sim_idx_col.push_back(col_idx[sim_num[k] % bn_c]);
                data_noisy.push_back(
                        image_noisy(Rect(sim_idx_col[k], sim_idx_row[k], kWien, kWien)));
            }
...
void getPatches(const Mat img, const int width, const int height, const int channels,
                const int patchSize, const int step, vector<Mat> &block, vector<int> &row_idx,
                vector<int> &col_idx) {
